...
function fitMolecule(atoms: Atom[], viewW: number, viewH: number, padding = 60) {
  ...
  const cw = maxX - minX;
  const ch = maxY - minY;
  const scale = Math.min(1.5, (viewW - padding * 2) / Math.max(cw, 1), (viewH - padding * 2) / Math.max(ch, 1));
...
export default function SimulationScreen() {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      fittedAtoms.forEach((atom, i) => {
        ...
        offsets[atom.id] = {
          dx: Math.sin(phase * 1.3) * intensity + (Math.random() - 0.5) * intensity * 0.5,
          dy: Math.cos(phase * 0.9) * intensity + (Math.random() - 0.5) * intensity * 0.5,
        };
      });
      ...
      setParticles(prev => prev
        .map(pt => ({ ...pt, x: pt.x + pt.vx, y: pt.y + pt.vy, vy: pt.vy - 0.02, life: pt.life + 1 }))
      ...
      if (p >= 1) {
        ...
        if (simulationResult === 'break') {
          ...
          for (let i = 0; i < 60; i++) {
            const src = fittedAtoms[Math.floor(Math.random() * fittedAtoms.length)];
            if (!src) continue;
            const angle = Math.random() * Math.PI * 2;
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      setParticles(prev => {
        const next = prev
          .map(pt => ({ ...pt, x: pt.x + pt.vx, y: pt.y + pt.vy, vy: pt.vy + 0.05, life: pt.life + 1 }))
          .filter(pt => pt.life < pt.maxLife);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center gradient-bg-subtle relative overflow-hidden">
      ...
      <div
        ...
        style={{
          background: `radial-gradient(circle at 50% 50%, hsla(${350 - progress * 50}, 80%, 50%, ${progress * 0.2}) 0%, transparent 70%)`,
        }}
      />
      ...
      <div
        ...
        style={{
          background: `radial-gradient(ellipse at center, transparent 40%, hsla(0, 0%, 0%, ${progress * 0.3}) 100%)`,
        }}
      />
      ...
      <div className="relative z-10 flex flex-col items-center gap-6 px-6 max-w-3xl w-full">
        ...
        <div className="relative w-full" style={{ maxWidth: '700px' }}>
          <svg viewBox={`0 0 ${SVG_W} ${SVG_H}`} className="w-full h-auto" ref={svgRef}>
            {/* Background glow */}
            <defs>
              <radialGradient id="heatGlow">
                <stop offset="0%" stopColor={`hsla(${350 - progress * 30}, 80%, 50%, ${progress * 0.25})`} />
            ...
            {particles.map((pt, i) => (
              <circle key={i}
                cx={pt.x} cy={pt.y} r={pt.r * (1 - pt.life / pt.maxLife)}
                fill={pt.color}
            ...
            {fittedBonds.map((bond) => {
              ...
              return (
                <line key={bond.id}
                  x1={from.x + offFrom.dx} y1={from.y + offFrom.dy}
                  x2={to.x + offTo.dx} y2={to.y + offTo.dy}
                  stroke={bondColor}
            ...
            {fittedAtoms.map((atom, i) => {
              ...
              return (
                ...
                >
                  {/* Outer energy glow */}
                  <circle
                    cx={atom.x + off.dx} cy={atom.y + off.dy}
                    r={glowRadius}
                    fill={progress > 0.5 ? `hsla(30, 80%, 50%, ${glowOpacity})` : atom.color}
                    opacity={glowOpacity}
                  />
                  {/* Inner glow ring */}
                  {progress > 0.4 && (
                    <circle
                      ...
                      fill="none"
                      stroke={`hsla(${40 - progress * 30}, 90%, 60%, ${progress * 0.4})`}
                      strokeWidth="1.5"
                    />
                  )}
                  {/* Main atom */}
                  <circle
                    ...
                    r={radius}
                    fill={atom.color}
                    stroke={`hsla(${40 - progress * 40}, 80%, 55%, ${progress * 0.7})`}
            ...
            <div className="h-2.5 rounded-full bg-muted/50 overflow-hidden backdrop-blur-sm">
              <div
                ...
                style={{
                  ...
                  boxShadow: progress > 0.5 ? `0 0 ${progress * 15}px hsla(30, 90%, 55%, 0.5)` : undefined,
                }}
              />
            </div>
