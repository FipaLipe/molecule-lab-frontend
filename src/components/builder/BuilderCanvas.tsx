...
export default function BuilderCanvas({
  ...
  const fitToContent = useCallback(() => {
    ...
    const allAtoms = showHydrogens ? [...atoms, ...implicitH.hydrogenAtoms] : atoms;
    const rect = svg.getBoundingClientRect();
    const padding = 80;
  ...
  const touchRef = useRef<{ dist: number; cx: number; cy: number; zoom: number; pan: { x: number; y: number } } | null>(null);
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    if (e.touches.length === 2 && touchRef.current) {
      ...
      setPan({
        x: mcx - zoomRatio * (mcx - touchRef.current.pan.x) + (cx - touchRef.current.cx),
        y: mcy - zoomRatio * (mcy - touchRef.current.pan.y) + (cy - touchRef.current.cy),
      });
      setZoom(newZoom);
  ...
  const cursorStyle = tool === 'pan' ? (isPanning ? 'grabbing' : 'grab')
    : tool === 'add' ? 'crosshair'
    : tool === 'move' ? (dragging ? 'grabbing' : 'grab')
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full">
      ...
      >
        ...
        <defs>
          <pattern id="grid" width={40 * zoom} height={40 * zoom} patternUnits="userSpaceOnUse"
            ...
            <circle cx={20 * zoom} cy={20 * zoom} r={0.7 * zoom} fill="hsl(var(--muted-foreground) / 0.2)" />
          </pattern>
        </defs>
        ...
        <g transform={`translate(${pan.x}, ${pan.y}) scale(${zoom})`}>
          ...
          {atoms.length === 0 && (
            <g>
              <text x="300" y="200" textAnchor="middle" dominantBaseline="central" fill="hsl(var(--muted-foreground))" fontSize="16" fontFamily="'Space Grotesk', sans-serif">
                Clique aqui para começar a construir
              </text>
              <text x="300" y="230" textAnchor="middle" dominantBaseline="central" fill="hsl(var(--muted-foreground) / 0.6)" fontSize="13">
      ...
      <div className="absolute bottom-3 right-3 flex flex-col gap-1.5 z-20">
        <button onClick={zoomIn} className="p-2 rounded-xl glass-card text-muted-foreground hover:text-foreground transition-all" title="Zoom in">
          <ZoomIn className="w-4 h-4" />
        </button>
        <button onClick={zoomOut} className="p-2 rounded-xl glass-card text-muted-foreground hover:text-foreground transition-all" title="Zoom out">
          <ZoomOut className="w-4 h-4" />
        </button>
        <button onClick={centerView} className="p-2 rounded-xl glass-card text-muted-foreground hover:text-foreground transition-all" title="Centralizar">
          <Crosshair className="w-4 h-4" />
        </button>
        <button onClick={fitToContent} className="p-2 rounded-xl glass-card text-muted-foreground hover:text-foreground transition-all" title="Ajustar à tela">
